...
**基于 AI 大模型的 A股／港股／美股／台股 智能分析系统**

自动分析自选股 → 生成决策仪表盘 → 多渠道推送（Telegram／Discord／Slack／邮件／企业微信／飞书）

**零成本部署** · GitHub Actions 免费运行 · 无需服务器

[**功能特性**](#-功能特性) · [**快速开始**](#-快速开始) · [**推送效果**](#-推送效果) · [**完整指南**](full-guide.md) · [**常见问题**](FAQ.md) · [**更新日志**](CHANGELOG.md)
...
| 市场 | 全球市场 | 支持 A股、港股、美股、台股 |
| 补全 | 智能补全 (MVP) | **[测试阶段]** 首页搜索框支持代码／名称／拼音／别名联想；本地索引已覆盖 A股、港股、美股、台股，并可通过 Tushare 或 AkShare 重新生成 |
| 复盘 | 大盘复盘 | 每日市场概览、板块涨跌、北向资金 |
| 回测 | AI 回测验证 | 自动评估历史分析准确率，方向胜率、止盈止损命中率 |
| **Agent 问股** | **策略对话** | **多轮策略问答，支持 11 种内置策略（Web／Bot／API）** |
| 推送 | 多渠道通知 | Telegram、Discord、Slack、邮件、企业微信、飞书等 |
...
| `SLACK_WEBHOOK_URL` | Slack Incoming Webhook URL（仅文字，不支持图片） | 可选 |
...
| `SERVERCHAN3_SENDKEY` | Server酱³ Sendkey（[获取地址](https://sc3.ft07.com/)，手机软件推送服务） | 可选 |
| `CUSTOM_WEBHOOK_URLS` | 自定义 Webhook（支持钉钉等，多个用逗号分隔） | 可选 |
| `CUSTOM_WEBHOOK_BEARER_TOKEN` | 自定义 Webhook 的 Bearer Token（用于需要认证的 Webhook） | 可选 |
| `SINGLE_STOCK_NOTIFY` | 单股推送模式：设为 `true` 则每分析完一只股票立即推送 | 可选 |
| `REPORT_TYPE` | 报告类型：`simple`（精简）或 `full`（完整），Docker 环境推荐设为 `full` | 可选 |
| `REPORT_LANGUAGE` | 报告输出语言：`zh`（默认中文）/ `en`（英文） | 可选 |
| `ANALYSIS_DELAY` | 个股分析和大盘分析之间的延迟（秒），避免 API 限流，如 `10` | 可选 |

> 至少配置一个渠道，配置多个则同时推送。更多配置请参考 [完整指南](full-guide.md)
...
| Secret 名称 | 说明 | 必填 |
|------------|------|:----:|
| `STOCK_LIST` | 自选股代码，如 `600519,hk00700,AAPL,TSLA,TW2330`（台股使用 `TW` + 代码，如 `TW2330`、`TW00050`） | ✅ |
| `TAVILY_API_KEYS` | [Tavily](https://tavily.com/) 搜索 API（新闻搜索） | 推荐 |
| `MINIMAX_API_KEYS` | [MiniMax](https://platform.minimaxi.com/) Web Search（结构化搜索结果） | 可选 |
| `BOCHA_API_KEYS` | [博查搜索](https://open.bocha.cn/) Web Search API（中文搜索优化，多个 key 用逗号分隔） | 可选 |
...
| `SEARXNG_BASE_URLS` | SearXNG 自建实例（需在 settings.yml 启用 format: json）；留空时默认自动发现公共实例 | 可选 |
| `SEARXNG_PUBLIC_INSTANCES_ENABLED` | 是否在 `SEARXNG_BASE_URLS` 为空时自动获取公共实例（默认 `true`） | 可选 |
| `TUSHARE_TOKEN` | [Tushare Pro](https://tushare.pro/weborder/#/login?reg=834638) Token | 可选 |
| `AGENT_MODE` | 启用 Agent 策略问股模式（`true`/`false`，默认 `false`） | 可选 |
| `AGENT_LITELLM_MODEL` | Agent 专用主模型（可选）；留空时继承 `LITELLM_MODEL` | 可选 |
| `AGENT_MAX_STEPS` | Agent 最大推理步数（默认 `10`） | 可选 |
| `AGENT_SKILLS` | 逗号分隔的策略技能 id；留空使用默认策略（内置默认为 `bull_trend`）；`all` 启用全部 | 可选 |
| `AGENT_SKILL_DIR` | 自定义策略技能目录（默认沿用内置 `strategies/` 路径） | 可选 |

**股票代码格式**

| 市场 | 格式 | 示例 |
|------|------|------|
| A股 | 6 位数字 | `600519`、`000001`、`300750` |
| 港股 | hk + 5 位数字 | `hk00700`、`hk09988` |
| 美股 | 1–5 位大写字母 | `AAPL`、`TSLA`、`GOOGL` |
| 台股 | TW + 4–6 位数字 | `TW2330`、`TW00050`、`TW2317` |
...
> 📖 本地运行、Docker 部署详细步骤请参考 [完整配置指南](full-guide.md)
...
> 📖 完整环境变量、定时任务配置请参考 [完整配置指南](full-guide.md)
...
- 🤖 **Agent 策略对话** — 启用 `AGENT_MODE=true` 后可使用 `/ask`、`/chat`、`/history`、`/strategies`、`/research` 进行问答、查历史、查看策略与深度研究
...
| `/api/v1/agent/skills` | GET | 获取可用策略技能列表（内置／自定义） |
| `/api/v1/agent/chat/stream` | POST (SSE) | Agent 多轮策略对话（流式） |
| `/api/health` | GET | 健康检查 |

> 备注：`POST /api/v1/analysis/analyze` 在 `async_mode=false` 时仅支持单只股票；批量 `stock_codes` 需使用 `async_mode=true`。

## 🔎 智能搜索补全 (MVP)

首页分析输入框现已升级为类搜索引擎的补全框，降低手动记忆股票代码的负担。

- **多维匹配**：支持股票代码、公司名称、拼音缩写与别名（例如 `gzmt` → 贵州茅台、`tencent` → 腾讯控股、`aapl` → Apple Inc.）。
- **多市场覆盖**：本地索引已覆盖 **A股、港股、美股、台股** 四个市场；需要时可基于 Tushare 或 AkShare 数据重新生成。
- **自动降级**：索引失效或加载失败时，界面自动退回一般手动输入模式，不阻断分析流程。

> 提示：如需更新索引，可先执行 `python3 scripts/fetch_tushare_stock_list.py`，再执行 `python3 scripts/generate_index_from_csv.py` 重新生成静态索引。
...
> 📢 以下功能将视后续情况逐步完成，如果你有好的想法或建议，欢迎 [提交 Issue](https://github.com/ct9915/daily_tw_stock_analysis/issues) 讨论！
...
- [x] 钉钉机器人（已支持命令交互 >> [相关配置](bot/dingding-bot-config.md)）
